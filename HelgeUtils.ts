...
*/
  ...
  export namespace Misc {
    ...
    // noinspection SpellCheckingInspection
    const wholeWordReplacementsRaw = [
          ["betreutest"     ,"betreute"               ],
          ["betreust"     ,"betreue"               ],
    ...
    export function compileReplacements(replacements: string[][], wordBoundaryAtStart: boolean) {
      return replacements.map(([duWort, ichWort]) => {
        const regex = (target: string) => {
          ...
          return new RegExp(`${
              maybeStartWordBoundary}${              target
              }\\b`, 'g');
        }

        const duUpper = Strings.toUppercaseFirstChar(duWort)
        const ichUpper = Strings.toUppercaseFirstChar(ichWort)

        return {
          lower: { regex: regex(duWort), replacement: ichWort },
          upper: { regex: regex(duUpper), replacement: ichUpper }
        }
      })
    }

    const wordEndReplacements = compileReplacements(wordEndReplacementsRaw, false)
    const wholeWordReplacements = compileReplacements(wholeWordReplacementsRaw, true)
    ...
    export const du2ich = (input: string) => {
      let output = input
      const apply = (replacements: { lower: { regex: RegExp, replacement: string }, upper: { regex: RegExp, replacement: string } }[]) => {
        for (const { lower, upper } of replacements) {
          output = output
              .replaceAll(lower.regex, lower.replacement)
              .replaceAll(upper.regex, upper.replacement)
        }
      }
      apply(wholeWordReplacements)
      apply(wordEndReplacements)
      return output
    }
